...
class FFANet(nn.Module):
    def __init__(self, num_groups=4, num_blocks=2, in_channels=64, out_channels=64, kernel_size=3, padding=1, remove_global_skip_connection=True):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding)
        self.groups = nn.ModuleList([
            nn.Sequential(*[
                ResidualBlockWithAttention(out_channels, out_channels) for _ in range(num_blocks)
                ])
            for _ in range(num_groups)
        ])
        self.remove_skip = remove_global_skip_connection
        self.attention = ResidualBlockWithAttention(in_channels=in_channels*num_groups, out_channels=out_channels, kernel_size=kernel_size, padding=padding)
